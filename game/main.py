...
class Player(pygame.sprite.Sprite):
    ...
    def update_sprite(self):          # for multiple type of sprites/ animations
        sprite_sheet = "idle"
        if self.hit:
            sprite_sheet = "hit"
        if self.y_vel < 0:
...
class Block(Object): #Block Maker
...
def handle_vertical_collision(player,objects,dy): # for hitbox collisions 
    collided_objects = []
    for obj in objects:
...
def main_menu(window):
    ...
    while run:
        ...
        title_text = font.render("Press Key to Start", True, (0, 0, 0))
...
def main(window):
    ...
    fire2 = Fire(550, HEIGHT - block_size - 450, 16 ,32)
    fire3 = Fire(700, HEIGHT - block_size - 450,16,32)
    Saws = Sawing(586, HEIGHT - block_size  - 80, 38 ,64)
    Saws2 = Sawing(1066, HEIGHT - block_size  - 470, 38 ,64)
    Saws2.on()
    Saws.on()
    fire3.on()
    fire.on()
    ...
    objects = [*floor, Block(0, HEIGHT - block_size * 2, block_size),
               ...
               fire , Block(block_size * 3, HEIGHT - block_size * 4, block_size),Saws
               ,Block(block_size * 4, HEIGHT - block_size * 5, block_size),
               Block(block_size * 5, HEIGHT - block_size * 5, block_size),fire2,
               ...
               Block(block_size * 7, HEIGHT - block_size * 5, block_size),fire3,
               Block(block_size * 8, HEIGHT - block_size * 5, block_size),
               Block(block_size * 9, HEIGHT - block_size * 5, block_size),Saws2,
